...
export function TabBar() {
  return (
    <nav
      aria-label="Primary"
      // The bar itself spans the window — a strip of chrome that stops
      // mid-screen looks like a rendering fault — but the tabs inside it sit in
      // the app's own column, so they stay a thumb's reach apart on a monitor.
      className={cx(
        "fixed inset-x-0 bottom-0 z-40 border-t border-border bg-chrome",
        "h-[calc(var(--size-tabbar)+env(safe-area-inset-bottom))]",
        "pb-[env(safe-area-inset-bottom)]",
      )}
    >
      <div className={cx(APP_WIDTH, "app-column-framed flex h-full justify-around")}>
        {TABS.map((tab) => (
          <NavLink
            key={tab.to}
            to={tab.to}
